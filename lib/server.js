...
function createServer(args) {
  ...
  const ctx = args.ctx;
  const POOL_SIZE = args.poolSize || 3;

  let clientId = 0;
  const clients = new Map();
  let cachedTemplateUrl = null;  // captured from any warmed page
      ...
      switch (msg.type) {
        case 'new':
          ...
          if (cachedTemplateUrl || (client.page && await client.page.evaluate(() => window.__m365CapturedUrl || null))) {
            ...
            if (tpl && !cachedTemplateUrl) cachedTemplateUrl = tpl;
