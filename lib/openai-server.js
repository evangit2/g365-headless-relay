...
function createOpenAiServer({ getTemplateUrl, semaphore, apiKey, port = 9000, host = '0.0.0.0', maxChars = DEFAULT_MAX_CHARS, requestLogPath = null }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (path === '/v1/models' && req.method === 'GET') {
      const data = Object.keys(MODEL_MAP)
        ...
        .map(id => ({ id, object: 'model', created: 0, owned_by: 'microsoft' }));
    ...
    if (totalChars > maxChars) {
      json(res, 400, { error: { message: `Prompt exceeds ${maxChars} character limit`, type: 'invalid_request_error' } });
      return;
    }
    ...
    const promptText = body.messages.map(m => {
      ...
      if (typeof m.content === 'string') text = m.content;
        ...
        text = m.content.map(part => typeof part === 'string' ? part : part.text || '').join('\n');
      }
      return `${m.role}: ${text}`;
    }).join('\n\n');
    ...
    try {
      ...
      json(res, 503, { error: { message: 'Server is at capacity; try again later', type: 'rate_limit_error' } });
      return;
    }
  ...
  server.listen(port, host, () => {
    ...
    console.log(`  GET  /v1/conversations`);
